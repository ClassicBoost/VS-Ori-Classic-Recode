...
class TitleState extends MusicBeatState
{
	...
	var disableMusicDupe:Bool = false;

	var inSelectMenu:Bool = false;
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menus/base/bg'));
		...
		logoBl = new FlxSprite(-150, 0).loadGraphic(Paths.image('menus/base/title/logo'));
		...
		startText = new FlxText(5, FlxG.height - 18, 0, "> START GAME <\nOPTIONS\n", 12);
		startText.scrollFactor.set();
		startText.screenCenter(X);
		startText.y = 500;
		startText.alpha = 0;
		startText.setFormat(Paths.font('alex.ttf'), 30, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(startText);
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			...
			new FlxTimer().start(1, function(tmr:FlxTimer)
			{
				...
				FlxTween.tween(startText, {alpha: 1}, 0.25, {ease: FlxEase.linear});
				// 
			});
			} else {
				switch (curSelected) {
					case 0:
						Main.switchState(this, new SelectGameState());
					case 1:
		...
		if (curSelected == 0) startText.text = '> START GAME <\nOPTIONS\n';
		if (curSelected == 1) startText.text = 'START GAME\n> OPTIONS <\n';
		startText.screenCenter(X);
