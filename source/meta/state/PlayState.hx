...
class PlayState extends MusicBeatState
{
	...
	private var disableFakeBFidle:Bool = false;
	...
	override public function create()
	{
		...
		bftwobutfake.setCharacter(1220, 1020,'ori');
		bftwobutfake.flipX = true;
		add(bftwobutfake);

		bfbutfake = new Character();
		bfbutfake.setCharacter(1110, 920,SONG.player1);
		add(bfbutfake);
		remove(bfbutfake);
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase() == 'decay') { // wait do I have to put caps in songs name
			switch (curStep) {
		...
		if (curSong.toLowerCase() == 'spirit-tree') {
			switch (curStep) {
				case 1:
		...
		if (curSong.toLowerCase() == 'trirotation') { // wait do I have to put caps in songs name
			switch (curStep) {
				case 4:
					gf.y = -1000;
				case 40,576:
				...
				case 384,768:
					...
					changePlayer();
				case 960,1008,1056:
					disableFakeBFidle = true;
					bftwobutfake.animation.play('singUP',true);
				case 984,1044,1104:
					disableFakeBFidle = true;
					bftwobutfake.animation.play('singDOWN',true);
				case 996,1032:
					disableFakeBFidle = true;
					bftwobutfake.animation.play('singRIGHT',true);
				case 1080,1036:
					disableFakeBFidle = false;
					bftwobutfake.animation.play('idle',false);
				case 1142:
					gf.visible = true;
					FlxG.camera.shake(0.5, 0.1);
					FlxG.sound.play(Paths.sound('slam'), 1);
					stageBuild.triggerEvent('normal');
					gf.color = 0xFF5B5E7D;
					boyfriend.color = 0xFF5B5E7D;
					boyfriend.animation.play('hey');
					FlxTween.tween(iconP2, {alpha: 0}, 0.5, {ease: FlxEase.linear});
					FlxTween.tween(dadOpponent, {alpha: 0}, 0.5, {ease: FlxEase.linear});
					FlxTween.tween(gf, {y: 300}, 0.2, {ease: FlxEase.linear});
			}
		}
	...
	function changePlayer(?idiot:String = 'ori') {
		if (idotChanged) {
			remove(bftwobutfake);
			bfbutfake = new Character();
			bfbutfake.setCharacter(1110,1020, SONG.player1);
			bfbutfake.flipX = false;
			if (Stage.mood == 'rain')
			bfbutfake.color = 0xFF2B2B2B;
			add(bfbutfake);
			boyfriend.setCharacter(1220, 1020, idiot);
			boyfriend.flipX = true;
			iconP1.updateIcon(idiot, true);
		} else {
			remove(bfbutfake);
			boyfriend.setCharacter(1110,1020, SONG.player1);
			boyfriend.flipX = false;
			if (Stage.mood == 'rain')
			boyfriend.color = 0xFF2B2B2B;
			bftwobutfake = new Character(); // switch bf
			bftwobutfake.setCharacter(1220, 1020, idiot);
			bftwobutfake.flipX = true;
			add(bftwobutfake);
	...
	override function beatHit()
	{
		...
		if (hasTWOplayers && !disableFakeBFidle) {
